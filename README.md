...
## 🛒 최근 업데이트 & 릴리즈 노트
**v1.23.0 (업데이트 수정)**
- **Disk Deep Clean기능 추가**: 디스크에서 각종캐시를 지워서 저장공간의 낭비를 줄입니다.
- **MacbookPro의 팬속도 확인 기능 추가**: MacBookPro등 내부 쿨링팬의 RPM을 확인 할 수 있습니다. 

**v1.22.0 (업데이트 수정)**
**v1.21.0 (메뉴바 항목 영속화 및 시스템 팬 모니터링 추가)**
- **메뉴바 항목 위치 저장(Autosave) 기능**: 사용자가 드래그하여 배치한 메뉴바 아이템들의 순서와 위치를 macOS가 기억하도록 `autosaveName`을 적용하여 재부팅이나 앱 재시작 후에도 위치가 유지되도록 수정했습니다.
- **쿨링 팬(Fan) 속도 모니터링 모듈 추가**: SMC 센서를 활용해 기기 내부 쿨링 팬의 회전 속도(RPM)를 감지하고, 실시간 그래프 및 팝오버를 통해 상세한 팬 상태를 확인할 수 있는 팬 모니터(`FanMonitor` 및 상세 뷰)를 새롭게 도입했습니다.
- **날짜 모듈의 순환 로직 분리**: 제한된 메뉴바 공간에서 CPU, 메모리 등은 일정 시간마다 순환하더라도 날짜 및 시간은 항상 고정된 상태로 확인할 수 있도록 날짜 모듈을 순환(Rotation) 대상에서 제외해 독립적으로 고정 표시되도록 개선했습니다.
- **매시 정각 강조 애니메이션 효과**: 매 시간 정각이 되었을 때 사용자가 인지하기 쉽도록 시간 텍스트 부분에 정각 알림 강조 애니메이션 효과를 추가했습니다.
- **팝오버 트리거 및 이벤트 처리 표준화**: 메뉴바 순환 모드의 수동 제어 버튼(`<`, `>`) 조작 시 상세 팝오버가 오작동하거나 겹쳐서 열리는 현상을 방지하기 위해 팝오버 호출 이벤트를 표준화하고 리팩토링했습니다.
- **성능 최적화 및 빌드 에러 수정**:
  - `DateItemView` 및 백그라운드 모니터링 주기를 정밀하게 다듬어 CPU 렌더링 오버헤드를 한층 줄였습니다.
  - Swift 컴파일 시 'Whole Module Optimization (WMO)' 환경에서 발생하던 컴파일 오류 구조를 수정하여 배포용 빌드 안정성을 확보했습니다.

**v1.20.0 (메뉴바 순환 모드 도입 및 UI 안정화)**
- **메뉴바 항목 순환 표시(Rotation Mode) 기능 추가**: 제한된 메뉴바 공간을 효율적으로 사용하기 위해 CPU, 메모리, 네트워크, 디스크 등 선택한 여러 모니터링 항목들을 일정 시간(1~10초)마다 자동으로 교차하여 보여주는 기능을 도입했습니다.
- **고정 너비 통합 컨테이너 디자인**: 항목 전환 시 개별 뷰의 너비 차이로 인해 메뉴바 전체 아이콘들이 좌우로 움직이는 현상을 방지하기 위해, 110px 크기의 고정 너비 컨테이너를 적용하여 시각적 안정성을 확보했습니다.
- **순환 제어 및 팝오버 즉시 연동**: 메뉴바 아이템 좌우에 수동 전환 버튼(`<`, `>`)을 배치하여 원하는 항목을 바로 찾을 수 있으며, 현재 표시 중인 정보 영역을 클릭하면 해당 모듈의 상세 팝오버 창이 즉시 노출됩니다.
- **환경설정 UI 고도화**: 설정창에 'Rotation Mode' 섹션을 신설하여 순환 On/Off, 대상 항목 선택, 순환 주기를 자유롭게 조절할 수 있도록 했으며, 늘어난 설정 항목에 대응하여 스크롤 뷰(Scroll View)를 적용했습니다.

**v1.19.0 (온도 센서 표기 최적화 및 빌드 안정성 향상)**
- **온도 센서 명칭 직관성 개선**: 메뉴바 팝오버에서 표시되는 온도 센서 이름 중 'NAND', 'gas gauge battery', 'tdev', 'tcal' 등 파악하기 어려운 기계적인 용어들을 'Memory(메모리)', 'Battery(배터리)', 'Power 1', 'Calibration' 등 알기 쉬운 일반적인 명칭으로 자동 매핑하여 표시하도록 로직을 전면 개선했습니다.
- **네트워크 상세 표기 성능 개선**: 네트워크 활성 서비스 표기 과정에서 발생하던 SwiftUI 내부 컴포넌트 렌더링 병목 구조를 최적화하여 뷰를 렌더링하는 컴파일 속도를 크게 향상했습니다. (컴파일러 Timeout 버그 해결)
- **macOS 빌드 호환성 복구**: macOS 13 전용 폰트 모디파이어를 하위 호환 지원 가능한 방식으로 재작성하고, C 심볼 중복 선언 현상을 해소하여 macOS 12 환경에서도 최적의 빌드 및 배포가 진행되도록 복원했습니다.
- **Liten중인 Process목록관리**: 현재 PC에서 Port를 Open하고 있는 Process목록을 조회하고 중단 시킬 수 있습니다.
- **NAT IP표시**: 현재 연결된 네트웍의 NAT IP를 확인 할 수 있습니다.

**v1.18.0 (시스템 호환성 및 다중 모니터 UI 개선)**
- **로그인 시 자동 시작(Start on Login) 시스템 호환성 개선**: macOS 최신 정책(`ServiceManagement` API) 지원을 적용하여, 환경설정에서 앱 자동 실행을 켜놓아도 실제 시스템 부팅 시 작동하지 않던 버그를 완벽하게 고쳤습니다. (macOS 13.0 이상 시스템 통합)
- **일정 알람 팝업창 잘림 현상 방지**: 메뉴바가 듀얼 모니터의 가장자리에 있거나 알람창 내용이 길어질 때, 일정 알람 팝업창 상단이 모니터 화면 밖이나 메뉴바 위로 비집고 나가 짤리는 문제를 해결했습니다. 창 내부 컨텐츠 크기를 사전에 정밀하게 계산한 뒤 메뉴바 바로 아래로 딱 맞게 내려오도록 뷰 좌표 로직을 전면 보정했습니다. 이제 알림창 내용이 높게 커져도 무조건 100% 화면 안에 안전하게 표출됩니다.

**v1.17.0 (디스크 표시 최적화 및 버그 수정)**
- **외장 디스크 표시 로직 개선**: 메뉴바에서 외장 디스크의 정보를 '여유 공간' 대신 **'사용 공간 / 전체 공간'**으로 표시하도록 변경하여 메인 팝업 메뉴와 일관성을 맞추었습니다.
- **표시 포맷 정밀도 향상**: 전체 용량 및 사용량 표시 시 소수점 첫째 자리까지 표기(`%.1fG`)하여 더 정확한 정보를 제공합니다.
- **메뉴바 공간 효율화**: 외장 디스크가 연결되어 있지 않은 경우, 메뉴바에서 불필요한 "None" 항목을 자동으로 숨기도록 최적화했습니다.
- **데이터 조회 정확도 보정**: `DiskMonitor`에서 시스템 API 사용 방식을 개선하여 일부 외장 저장 장치에서 사용량이 0.0G로 잘못 표시되는 현상을 방지했습니다.

**v1.16.0 (업데이트 자동 체크)**
- **1시간 간격으로 업데이트 체크**
**v1.15.0 (메모리 최적화 및 빌드 안정성 개선)**
- **장치 모니터링 메모리 누수(Memory Leak) 전면 수정**: 장시간 앱 실행 시 메모리 점유율이 비정상적으로 높아지던 6가지 주요 누수 패턴을 해결했습니다.
  - **IOReport/HID 센서 리소스 관리**: 매초 생성되던 `CFDictionary`, `CFArray` 등 저수준 시스템 객체의 retain/release 로직을 정밀하게 보정하여 메모리 누적을 완벽히 차단했습니다.
  - **SSID 조회 방식 최적화**: 10초마다 외부 프로세스(`networksetup`)를 실행하던 방식을 네이티브 프레임워크(`CWWiFiClient`) 호출로 변경하여 CPU 오버헤드와 파일 핸들 누수를 모두 제거했습니다.
  - **상태바 아이템 렌더링 리팩토링**: 설정 변경 시 상태바 아이템을 매번 삭제·재생성하던 방식에서, 최초 1회 생성 후 가시성(`isVisible`)만 조절하는 방식으로 개선하여 불필요한 객체 생성을 막았습니다.
  - **타이머 및 네트워크 히스토리 관리**: 모니터링 타이머의 순환 참조를 제거하고, VPN 등 비활성 네트워크 인터페이스의 잔여 데이터를 자동 정리하는 로직을 추가했습니다.
- **하위 macOS 버전 빌드 호환성 해결**: 최신 macOS 전용 구문을 하위 호환 가능한 코드로 변경하여, `build.sh`를 통한 배포 환경(macOS 12.0+)에서의 빌드 오류를 해결했습니다.
...
- **메뉴막대(Menu Bar) GPU 전력 / 온도 위젯 추가**: 앱 환경설정(⚙️) 창의 'Monitoring Items' 트리에 `GPU Usage`와 `Temperature Status` 단축 위젯(토글) 스위치를 신설했습니다. 이것을 활성화하면 사용 중인 맥 화면 최상단 상태표시줄(메뉴 바)에 실시간 GPU 와트(W) 소비량과 내부 온도(°C)가 아기자기한 모듈 폼(Pill)으로 즉각 반영되며, 터치 시 즉시 메인 팝오버 영역으로 화면이 열립니다.
